...
/**
 * The role the stage delegates to.
 *
 ...
 *
 * No tools and no persistence. It exists to be spawned, answer one line, and be
 * disposed - and a demonstration agent that could touch the disk would be the
 * one part of the tour that is not safe to run twice.
 */
export function registerDrillProfile(api: ExtensionActivationApi): void {
	api.registerProfile({
		...
		whenToUse: "Only inside a drill. It has no tools and its session is not kept.",
