...
// content is optional so an AgentMessage[] - a union whose bash-execution
// member carries no content - can be passed directly.
...
describe("AgentHarness", () => {
	...
	it("promotes queued follow-ups into the steering queue without duplicating them", async () => {
		const registration = newFaux();
		let releaseFirstResponse: (() => void) | undefined;
		const firstResponseReleased = new Promise<void>((resolve) => {
			releaseFirstResponse = resolve;
		...
		expect(queueUpdates).toContainEqual({ steer: 1, followUp: 0 });
		// The promoted message is read at the turn boundary, and only once.
		expect(secondRequestText).toEqual(["first", "later"]);
	});

	it("rolls back only unconsumed promoted messages when a queue observer fails", async () => {
		const registration = newFaux();
		...
		const promotionObserved = deferred();
		const rejectPromotionUpdate = deferred();
		let shouldRejectPromotion = true;
		harness.subscribe(async (event) => {
			if (
				event.type !== "queue_update" ||
				!shouldRejectPromotion ||
				event.followUp.length !== 0 ||
				!textFromUserMessages(event.steer).includes("later")
			) {
				return;
			}
			shouldRejectPromotion = false;
			promotionObserved.resolve();
			await rejectPromotionUpdate.promise;
			throw new Error("promotion observer failed");
		});

		const promotion = harness.promoteFollowUpsToSteer();
		const rejectedPromotion = expect(promotion).rejects.toThrow("promotion observer failed");
		await promotionObserved.promise;
		await harness.steer("urgent");
		rejectPromotionUpdate.resolve();
		await rejectedPromotion;

		releaseFirstResponse.resolve();
		await firstPrompt;

		expect(requestText).toEqual([["first"], ["first", "urgent"], ["first", "urgent", "later"]]);
	});
