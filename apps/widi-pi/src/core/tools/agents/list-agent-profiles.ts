...
> {
	return {
		...
		description:
			"List the agent profiles that can be used with spawn_agent. Each profile is a role: it decides the spawned agent's system prompt, tools, and whether its session persists. Call this before spawn_agent instead of guessing a profile name.",
...
function formatProfiles(profiles: readonly AgentProfileBrief[]): string {
	...
	const lines = profiles.map((profile) => {
		...
		const session = profile.persist
			? "persistent session"
			: "ephemeral session";
		return `- ${profile.id} (${profile.label})${description} [${session}]`;
	});
