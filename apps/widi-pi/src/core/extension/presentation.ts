...
export const EXTENSION_MESSAGE_KINDS = [
	"text",
	"markdown",
	"code",
	"table",
...
export type ExtensionTableAlignment =
	(typeof EXTENSION_TABLE_ALIGNMENTS)[number];
...
export const EXTENSION_STATUS_REGIONS = [
	"panel",
	"footer",
	"agent-strip",
] as const;
...
export function validateExtensionStatus(
	...
	if (progress !== undefined) {
		...
		if (progress.total === undefined) {
			...
			assertNonNegativeInteger(
				progress.total,
				"Extension status progress total",
			);
	...
	if (status.region !== undefined) {
		if (
			!(EXTENSION_STATUS_REGIONS as readonly string[]).includes(status.region)
		) {
			throw new TypeError(
				`Extension status region must be one of: ${EXTENSION_STATUS_REGIONS.join(", ")}.`,
			);
		}
...
function validateStatusIcon(icon: string): string {
	...
	if (utf8ByteLength(icon) > MAX_EXTENSION_STATUS_ICON_BYTES) {
		throw new RangeError(
			`Extension status icon exceeds ${MAX_EXTENSION_STATUS_ICON_BYTES} UTF-8 bytes.`,
		);
	}
	if (CONTROL_CHARACTER_PATTERN.test(icon)) {
		throw new TypeError(
			"Extension status icon must not contain control characters.",
		);
	}
...
function validateTone(tone: ExtensionTone, label: string): ExtensionTone {
	if (!(EXTENSION_TONES as readonly string[]).includes(tone)) {
		throw new TypeError(
			`${label} must be one of: ${EXTENSION_TONES.join(", ")}.`,
		);
	}
	return tone;
}
...
export function validateExtensionMessage(
	...
	if (size > MAX_EXTENSION_MESSAGE_BYTES) {
		throw new RangeError(
			`Extension message exceeds ${MAX_EXTENSION_MESSAGE_BYTES} UTF-8 bytes.`,
		);
	}
	...
	switch (message.kind) {
			...
			return {
				...
				path:
					message.path === undefined
						...
						: assertBoundedNonBlankText(
								message.path,
								"Extension message path",
								MAX_EXTENSION_MESSAGE_PATH_BYTES,
							),
		...
		case "banner":
			return {
				kind: "banner",
				title,
				severity: validateTone(
					message.severity,
					"Extension message banner severity",
				),
				content: validateMessageContent(message.content),
			};
	}
...
function validateMessageContent(
	content: string,
	label = "Extension message content",
): string {
	...
	if (utf8ByteLength(content) > MAX_EXTENSION_MESSAGE_CONTENT_BYTES) {
		throw new RangeError(
			`${label} exceeds ${MAX_EXTENSION_MESSAGE_CONTENT_BYTES} UTF-8 bytes.`,
		);
	}
	return content;
}
...
function validateMessageLanguage(
	language: string | undefined,
): string | undefined {
	if (language === undefined) return undefined;
	if (
		typeof language !== "string" ||
		!EXTENSION_MESSAGE_LANGUAGE_PATTERN.test(language)
	) {
		throw new TypeError(
			"Extension message language must contain only letters, numbers, '.', '_', '+', '#', and '-'.",
		);
	}
	if (utf8ByteLength(language) > MAX_EXTENSION_MESSAGE_LANGUAGE_BYTES) {
		throw new RangeError(
			`Extension message language exceeds ${MAX_EXTENSION_MESSAGE_LANGUAGE_BYTES} UTF-8 bytes.`,
		);
	}
	return language;
}

function validateTableMessage(
	message: ExtensionTableMessage,
	title: string | undefined,
): ExtensionTableMessage {
	...
	if (message.columns.length > MAX_EXTENSION_MESSAGE_TABLE_COLUMNS) {
		throw new RangeError(
			`Extension message columns exceed ${MAX_EXTENSION_MESSAGE_TABLE_COLUMNS} entries.`,
		);
	}
	const columns = mapDenseArray(
		message.columns,
		"Extension message columns",
		(column): ExtensionTableColumn => {
			...
			assertBoundedText(
				column.label,
				"Extension message column label",
				MAX_EXTENSION_MESSAGE_CELL_BYTES,
			);
			if (
				column.align !== undefined &&
				!(EXTENSION_TABLE_ALIGNMENTS as readonly string[]).includes(
					column.align,
				)
			) {
				throw new TypeError(
					`Extension message column align must be one of: ${EXTENSION_TABLE_ALIGNMENTS.join(", ")}.`,
				);
			}
	...
	if (message.rows.length > MAX_EXTENSION_MESSAGE_TABLE_ROWS) {
		throw new RangeError(
			`Extension message rows exceed ${MAX_EXTENSION_MESSAGE_TABLE_ROWS} entries.`,
		);
	}
	...
	const rows = mapDenseArray(message.rows, "Extension message rows", (row) => {
		if (!Array.isArray(row) || row.length !== columns.length) {
			throw new TypeError(
				`Extension message row must be an array of ${columns.length} cells.`,
			);
		}
		return mapDenseArray(row, "Extension message row", (cell) => {
			assertBoundedText(
				cell,
				"Extension message cell",
				MAX_EXTENSION_MESSAGE_CELL_BYTES,
			);
			return cell;
		});
...
function validateFieldsMessage(
	message: ExtensionFieldsMessage,
	title: string | undefined,
): ExtensionFieldsMessage {
	...
	if (message.fields.length > MAX_EXTENSION_MESSAGE_FIELDS) {
		throw new RangeError(
			`Extension message fields exceed ${MAX_EXTENSION_MESSAGE_FIELDS} entries.`,
		);
	}
	const fields = mapDenseArray(
		message.fields,
		"Extension message fields",
		(field): ExtensionMessageField => {
			if (typeof field !== "object" || field === null) {
				throw new TypeError("Extension message field must be an object.");
			}
			assertBoundedNonBlankText(
				field.label,
				"Extension message field label",
				MAX_EXTENSION_MESSAGE_CELL_BYTES,
			);
			assertBoundedText(
				field.value,
				"Extension message field value",
				MAX_EXTENSION_MESSAGE_CELL_BYTES,
			);
			return {
				label: field.label,
				...
				tone:
					field.tone === undefined
						? undefined
						: validateTone(field.tone, "Extension message field tone"),
			};
		},
	);
