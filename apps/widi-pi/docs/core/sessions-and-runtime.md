...
`SessionFactStore` 是 tool/runtime facade，不是新的 session backend。`SessionManager` 仍决定当前 agent 使用 persistent JSONL session 还是 ephemeral in-memory session；orchestrator 只在 harness build 时把这个真实 `Session` 包成 `SessionBackedSessionFactStore` 并注入 tool context。普通 tool result、assistant message、active tools change 等仍由 Pi `AgentHarness` 自己写入 session。

当前 session fact persistence 已经落地，但恢复语义还未完成。后续需要明确 branch scope、payload schema、fact definition 生命周期、diagnostics、compaction/export policy，以及 extension namespace 的隔离规则。在这些规则完成前，session fact 只适合保存小型、低风险、可丢失后重建或降级的辅助事实。
...
- 不把 session fact 当作替代 Pi tool result 或 message history 的写入通道。
...
- [x] 实现基于 Pi `custom` entry 的 session fact persistence。
- [ ] 定义 session fact 的 branch scope 和 fork 语义。
- [ ] 定义 fact definition 注册、resume 恢复顺序和恢复诊断。
- [ ] 定义 session fact payload 的 schema、大小、序列化和敏感信息 policy。
- [ ] 定义 compaction、export 和 debug view 对 session facts 的处理策略。
