...
- [x] 将 historical `write`、`read`、`bash` examples 的 Pi 风格 tool call/result/details 行为记录到 `core-tools.md`。
- [ ] 后续若重新设计 coding tool/backend，再为新实现明确 result/details 约定；不继续扩展当前 legacy `bash/read/write`。
