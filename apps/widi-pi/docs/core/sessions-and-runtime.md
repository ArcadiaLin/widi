...
Historical `write`、`read` 和 `bash` examples 已从仓库删除；`read`/`bash`/`edit`/`write`/`grep`/`find`/`ls` 现为 `src/core/tools/coding/` 下的真实 core 实现。Coding tools 的可恢复数据仍应通过 Pi tool call/result 表达：正文或输出进入 tool call/result，结构化信息进入 typed `details`，不通过 core tool state 或 custom entry 写入额外恢复数据。
...
`HumanRequestBroker` 持有 pending request、timeout、abort 和 cancel 状态；这些都是 runtime-local state，不是可恢复 session state。只有当 human-request 作为 tool call 的结果发生时，它才自然作为 tool result 进入 session。其他 human interaction 属于 orchestrator runtime 层，不是 session 管理重点。
...
- 不把 pending human request 等同于 session state。
