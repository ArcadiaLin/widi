...
| `setSessionName` / `getSessionName` | scoped `setSessionName`/`getSessionName` 已暴露 | core，已落（set 随切片 3；get 曾漏落，2026-07-13 审计补齐） |
| `setLabel` | pi session label，WIDI 无对应 | backlog（upstream 对齐项） |
| `exec` | scoped `exec` 已暴露，project trust 门控 | core，已落（切片 3，trust 裁决见切片记录） |
| `getActiveTools` / `getAllTools` / `setActiveTools` | scoped `getTools`/`setTools`/`setActiveTools` | core，已落但窄于 pi（2026-07-13 审计修正表述）：`getTools` 只回名字集（`AgentToolsSnapshot`），pi `getAllTools` 含 parameters schema、promptGuidelines 与 provenance；tool 元数据查询面 backlog 待举证 |
| `getCommands` | scoped `getCommands` 已暴露 | core，已落（切片 3，与 client 同一门控口径） |
| `setModel` / `get/setThinkingLevel` | scoped `setModel`/`getModel`/`getThinkingLevel`/`setThinkingLevel` 已暴露 | core，已落（切片 3；`getModel` 随 2026-07-13 审计补遗） |
| `events`（extension 间 EventBus） | 无 | backlog（无 consumer 举证） |
| `ctx.ui` / `hasUI` / `mode` | 无 | client；core 的对应物是 `requestHuman` |

### 运行时 context 面（ctx.*，2026-07-13 审计补录）

原对照表只审计了 pi `ExtensionAPI` 方法面；pi `ExtensionContext`/`ExtensionCommandContext` 的运行时能力此前未逐项落归属，随全面审计补录：

| pi ctx 能力 | WIDI 现状 | 归属 |
| --- | --- | --- |
| `ctx.model`（当前模型） | scoped `getModel()` | core，已落（审计补遗接线，`getAgentModel` 原有） |
| `ctx.modelRegistry`（枚举模型） | scoped `listModelCandidates()`，候选 `value` 即 `setModel` reference | core，已落（审计补遗接线） |
| `ctx.abort()` | scoped `abort()`；audit extension kill-switch（`abortOn` 策略）为锚点 consumer | core，已落（审计补遗接线） |
| `ctx.compact(options)` | scoped `compact(customInstructions?)`；pi 的 onComplete/onError 回调不做，完成事实经 raw `session_compact` | core，已落（审计补遗接线） |
| `ctx.isIdle()` / `ctx.signal` | context `isIdle()` / `signal` | core，已落（切片 3） |
| `ctx.hasPendingMessages()` | raw `queue_update` observer 可达 | core，已落（raw 口径） |
| `ctx.getSystemPrompt()` / command ctx `getSystemPromptOptions()` | `before_agent_start` event 携带 systemPrompt，hook 时可达；on-demand getter 无 | 差异接受（待举证） |
| `ctx.sessionManager`（只读 session 历史） | 无；extension 只能读自己的 custom entries，消息只在 `context` interceptor 流经时可见 | backlog（「总结会话」类 consumer 举证后定只读 facade 形态） |
| `ctx.getContextUsage()` | 无 | backlog（compaction 策略 extension 举证，需先定 token 事实来源） |
| `ctx.cwd` | 无（`exec` 在项目 cwd 执行，事实本身不暴露） | 差异接受（待举证） |
| `ctx.isProjectTrusted()` | 无；受门控 action 失败时以 `extension.exec_denied` 等 diagnostic 解释 | 差异接受 |
| `ctx.shutdown()` | 无；进程生命周期归 runtime owner | 差异接受（裁决：extension 不得终止宿主进程） |
| `ctx.ui.notify` / `setStatus`（轻量单向用户提示） | 无 core 对应物；custom entry / 事件由 client 渲染 | client（adapter extension host 承载；UI 减法补偿义务的补遗格） |
| command ctx `newSession` / `fork` / `navigateTree` / `switchSession` | orchestrator 原子方法在（fork/navigateTree），未暴露给 extension | backlog（session 控制权与 M3 spawn/collaboration 门控一并裁决） |
| command ctx `reload()` | `reloadExtensions` 属 runtime owner 面 | 差异接受（extension 自 reload 无场景） |

另一处补录：pi coding-agent 的 `message_end` 结果可**替换**最终 assistant 消息（mutate 档，脱敏类场景）；WIDI 的 `message_*` 只有 raw observe，且 pi harness 无此 hook——收编需 WIDI 自建档位。差异记录，backlog 待举证。
...
- **新 backlog 举证缺口**：session 只读 facade、`getContextUsage`、`message_end` mutate 档、tool 元数据查询面、轻量 notify 通道（归 client host）。
