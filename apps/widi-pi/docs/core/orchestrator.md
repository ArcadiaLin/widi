...
已落地的插入点（代码锚点见 `extension/runner.ts` 与 [Runtime Lifecycle](./runtime-lifecycle.md) 的 Extension Execution Points）：
...
Orchestrator 后续任务按 milestone 维护在 [Milestones](../TODO.md) 与 [Backlog](../BACKLOG.md)。模块执行顺序和事件传递边界见 [Runtime Lifecycle](./runtime-lifecycle.md)。本文件只保留 runtime coordinator 的职责边界。
