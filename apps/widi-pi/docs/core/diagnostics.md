...
Profile registry、tool registry 和 resource loader 不必一次性重写内部类型。它们可以先提供 adapter：

- `toCoreDiagnosticFromProfile(...)`
- `toCoreDiagnosticFromTool(...)`
- `toCoreDiagnosticFromResource(...)`
...
- [ ] 实现 profile/tool diagnostic adapter scaffolding。
