...
Session fact 不是 header metadata，也不是 extension-owned storage 的替代品。Pi storage 负责把底层 `custom` entry 原样保存和读回；WIDI 的 `SessionBackedSessionFactStore` 基于当前 agent 的真实 Pi `Session` 追加和查询这些 entries，因此持久 session 进入 WIDI JSONL，临时 session 进入 Pi in-memory repo。如果没有注册 `SessionFactDefinition.restore`，WIDI 也只把它作为原始 fact 暴露，不会自动恢复 typed runtime state。已注册的 fact definition 可以按 namespace、source、sourceName、factType 和 version 在 resume 时恢复 typed state。

`SessionFactStore` 由 orchestrator 在构建 harness 时注入 tool execution context。Tool Registry 不持有 session，也不直接接触 storage；它只把 resolved `ToolDefinition` wrap 成 Pi `AgentTool`。Tool 执行时拿到的 `context.session` 是当前 agent session 的 facade，底层仍调用 Pi `Session.appendCustomEntry()`。

当前实现仍有一些需要继续收敛的隐患：

- `find()` 目前读取整棵 session tree 的 custom entries，不区分当前 branch path、历史分支或 fork 来源。
- `SessionFactDefinition` 只是声明和手动 restore API，resume 时还没有统一的自动恢复顺序、作用域或 diagnostics。
- payload 只做最小 shape 校验，还没有强制 JSON serializable、大小限制、敏感信息扫描或 schema validation。
- namespace 依赖约定，尚未定义 extension/core namespace 的冲突策略。
- storage 写入失败会以 tool execution error 形式暴露，尚未转换成统一 `CoreDiagnostic`。
- compaction、branch summary 和 future export 是否应保留、隐藏或摘要 session facts 还没有 policy。
...
- 不把 `SessionFactStore` 用作绕过 Pi `AgentHarness` 标准 tool result/session history 的通道。
...
- [x] 实现基于 Pi `custom` entry 的持久化 `SessionFactStore`。
- [ ] 定义 `SessionFactStore.find()` 的 branch scope：整棵 tree、当前 branch path、或显式 query scope。
- [ ] 定义 fact definition 在 resume 时的自动恢复顺序、错误 diagnostics 和 extension 作用域。
- [ ] 为 session fact payload 增加 JSON serializable、大小限制、schema validation 和敏感信息 policy。
- [ ] 定义 extension/core session fact namespace 的冲突策略。
- [ ] 将 session fact 写入/恢复失败接入统一 `CoreDiagnostic`。
- [ ] 定义 compaction/export 对 session facts 的处理策略。
