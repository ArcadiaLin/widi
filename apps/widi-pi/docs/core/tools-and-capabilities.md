...
Tool tracker 是 `core/tools` 下的 runtime-only 状态，不写 session，也不做 queue、retry、scheduler、artifact manager 或 process spawn。它只记录当前进程内的 tool execution run，让 orchestrator、UI/RPC、future wait/result tool 或 extension 能查询正在执行或已经结束的 tool run。

所有 resolved tools 默认通过 registry adapter 进入 tracker。默认 tracking mode 是 `minimal`，只记录：

- tracking id、tool call id、tool name、source。
- running/succeeded/failed 状态。
- started/ended timestamp 和 duration。

默认不记录完整 params、stdout、stderr 或 result，避免泄漏和体积问题。Tool 或 extension 可以通过 `tracking` policy 自定义记录内容：

- `tracking: false` 关闭自动 tracking。
- `mode: "metadata" | "tail" | "full"` 表示 tool/extension 的记录意图。
- `describeParams`、`describeUpdate`、`describeResult`、`describeError` 从 params、streaming update、result 或 error 中抽取可记录数据。

Tracker 记录由 registry adapter 自动完成：execute 前 start，`onUpdate` 时 update，成功时 finish，抛错时 fail 并继续抛出原错误。没传 tracker 时使用 no-op tracker，tool 和 extension 不需要额外分支。

这块是 core 允许的最后一类 tool runtime state。复杂 job/workflow、远程 worker、stdout streaming backend、artifact 管理和调度策略应由 extension 或 runtime provider 实现。
...
- [x] 增加 runtime-only tool tracker，并在 registry adapter 中默认追踪 tool execution。
- [ ] 为 built-in tool wrapper 增加示例，先覆盖 `write` 的 sandbox/backend 替换场景。
- [ ] 设计基于 Pi `custom` entry 的 session fact API 和 fact definition 恢复规则。
