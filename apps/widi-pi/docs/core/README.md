...
- [x] 补齐 `SettingManager` 基础 runtime，覆盖 global/project settings、project trust、merge、flush 和 typed getters/setters。
- [ ] 每个分文档落地对应机制的最小术语表，并链接到根 `CONTEXT.md`。
- [ ] 梳理 core capability 列表，确认哪些能力可以被 tool adapter、extension hook 和 runtime adapter 调用。
