...
- Activation API 支持 `registerTool()`、`patchTool()`、`registerCommand()`、`contributeResources()`、`observe()` 和 `intercept()`。
...
这些能力足够验证 ToolRegistry、首批 hook matrix、diagnostics、reload、input command、session custom entry 和 resource contribution 的主路径。仍不足以作为稳定第三方 extension surface：provider registration、可取消 session/provider hooks、product presentation 和完整 RPC adapter 尚未收口。
