...
面向作者的开发指南（如何写一个 extension、能注册什么、推荐做法）见 [Extension Authoring](../extension-authoring.md)；本文记录机制边界与裁决。
...
- Missing extension / version mismatch / restore failed：条目原样保留。custom entry 是 session 的存储事实，不随 extension 可用性删除或隐藏；持久 type `extension:<extensionId>:<localType>` 经 `getAgentSessionTree` 仍可达，只是无人再以 `findEntries` 消费。`extension.version_incompatible` 语义已随 ME 切片 10 定案（见公开契约节）；孤儿条目如何向用户展示归 client adapter（举证缺口见 BACKLOG）。
...
- `ExtensionLoader` 支持内存 extension registry：`registerExtension(extensionId, module)`（裸 factory 或 `{ apiVersion, activate }` 定义）。
...
- Activation API 支持 `registerTool()`、`patchTool()`、`registerCommand()`、`contributeResources()`、`registerProvider()`、`observe()` 和 `intercept()`。
...
- Orchestrator 已将 `before_agent_start`、`before_provider_request`、`context`、`tool_call`、`tool_result` 五个 harness hook 桥接到 interceptors；`inputAgent` 直接驱动 WIDI 原生 `input` interceptor（契约见 Input 拦截契约节）。
...
- `agent.inspect` 已能暴露 loaded extensions、registered hooks、commands、tool contributions、resource contributions、patches、diagnostics 和 stale state；agent snapshot 另含 resolved resource provenance（每个已装载 skill/prompt template 的名字与来源 root/extension）。
- Extension 资源贡献经 `contributeResources()` 声明（ME 切片 8，契约见下文 Extension resource contribution 节）。
- Extension provider 贡献经 `registerProvider()` 声明（ME 切片 9，契约见下文 Extension provider contribution 节）。
- 作者 API 面冻结为 `extension/api.ts`（API v1，ME 切片 10）：版本声明 `{ apiVersion, activate }` 统一三种 source，不兼容版本在 load/注册期拒绝并发 `extension.version_incompatible`（契约见下文公开契约节）。
...
这些能力足够验证 ToolRegistry、首批 hook matrix、diagnostics、reload、input command、session custom entry、resource contribution 和 provider contribution 的主路径。作者契约已随 ME 切片 10 冻结为 API v1（见下节）；可取消 session hooks、`before_provider_payload`、product presentation 和完整 RPC adapter 留在 backlog 待举证。

### 公开契约与版本（API v1，ME 切片 10）

作者 API 的唯一 import 面是 `extension/api.ts`：第三方 extension 只 import 该模块与下列枚举的上游类型；`extension/index.ts` 是 core 内部 barrel（loader/runner/snapshot/`ExtensionCoreActions` 等绑定面），不属于作者契约。`api.ts` 的导出即公开契约清单：

- 版本：`EXTENSION_API_VERSION`、`MIN_SUPPORTED_EXTENSION_API_VERSION`、`isSupportedExtensionApiVersion()`。
- 激活面：`ExtensionFactory`、`ExtensionDefinition`（`{ apiVersion, activate }`）、`ExtensionModule`、`ExtensionActivationApi`（`registerTool`/`patchTool`/`registerCommand`/`contributeResources`/`registerProvider`/`observe`/`intercept`）。
- 定义类型：command 定义族（line/inline、arguments、handler/expand）、`ExtensionResourcePaths`、`ExtensionProviderConfig`、tool 契约 7 类型。
- 回调 context：`ExtensionContext`/`ExtensionCommandContext`、`ExtensionActions`、`ExtensionSessionContext`/`ExtensionCustomEntry`、`ExtensionExecResult`。
- 事件面：`ExtensionObservedEvent` 族、`ExtensionInterceptor` 族、`ExtensionInputEvent`/`ExtensionInputResult`。
- 复出口的 core 事实类型（作者不再 import core 内部模块）：`Command`、`CommandCandidates`、`CommandPlacement`、`CoreDiagnostic`、`HumanRequestDraft`/`HumanResponse`、`AgentToolsSnapshot`、`RuntimeModel`。

上游类型按引用冻结、不包装（raw 是唯一口径的推论）：Pi typed hook event/result（`BeforeAgentStartEvent` 等与 `AgentHarnessEventResultMap`）、raw `AgentHarnessEvent`、`ImageContent`、`ThinkingLevel`、`ShellExecOptions`、`Result`/`ExecutionError` 与 typebox `TSchema`/`Static` 属于契约面；它们的破坏性变更即 extension API 破坏性变更。

版本策略：单调递增整数，只在冻结面破坏性变更时 bump；runtime 支持区间为 `[MIN_SUPPORTED_EXTENSION_API_VERSION, EXTENSION_API_VERSION]`（当前 `[1, 1]`）。声明形态统一三种 extension source（factory/file/package）：模块默认导出或 `registerExtension` 传 `{ apiVersion, activate }`。裸 factory 视为面向当前版本——仓库内 extension 与 runtime 同步演进，强制声明只有噪音；第三方发布应当显式声明。

`extension.version_incompatible` 语义：版本检查发生在 load/注册期（版本是 extension 的属性，不是 (extension, agent) 的属性），不兼容的 extension 不进激活表并发 load 期诊断（error）。agent 引用它时 `loadForAgent` 发 per-agent 同 code 诊断（error、blocked，含 `declaredApiVersion` 与 `supportedApiVersions`），归因为真实原因而非 `factory_missing`，且不受 profile `missingExtensionSeverity` 调节。blocked 档意味着依赖它的 agent spawn/resume 失败——与 `activation_failed` 同族的 fail-closed：profile 声明的 extension 依赖无法满足时不静默降级。孤儿 custom entry 的展示归 client adapter（见 custom entry 契约节）。

验收锚点：`tests/extensions/third-party-extension.ts` 只 import `extension/api.ts` 与契约枚举的上游类型，组合 tool + line/inline command + observer 完成集成回归。
...
| --- | --- | --- | --- | --- |
| Pi agent/turn/message/tool execution、compact/tree、model/thinking update、`after_provider_response` | raw `agent_harness_event` | observe | 返回值忽略；失败产生 `extension.handler_failed`，后续 observer 继续 | own agent |
...
| `before_provider_request` | Pi typed hook | mutate | streamOptions patch 管线合成；失败跳过（塑形 hook，非门禁） | own agent |
...
- `before_provider_request`：管线式 streamOptions patch。后一个 handler 接收当前已合成的 streamOptions（含 header/metadata 的 key 级增删）；runner 把整条管线的净变化编码为单个 base-to-final patch 交还 Pi harness。
...
`before_agent_start`、`before_provider_request`、`context`、`tool_result` handler 抛错时，runner 产生 `extension.handler_failed` diagnostic，只丢弃该 handler 的结果并继续执行后续 handler；此前与此后的成功结果都保留。多个失败分别产生 diagnostic。若没有任何成功 handler 返回结果，harness 按无拦截结果继续。`before_provider_request` 失败跳过是裁决而非疏漏：它是请求塑形 hook（header/metadata/超时/重试），不是门禁——需要门禁语义的策略应拦 `tool_call`/`input`，或在 backlog 为 provider 门禁举证。
...
### Extension resource contribution（ME 切片 8）

`contributeResources({ skillPaths, promptTemplatePaths })` 在激活期声明额外的 skill / prompt template 路径：

- **形态**：路径声明，不是对象注册。extension 只交出目录/文件路径，ResourceLoader 仍是唯一的文件系统读取者与解释者（skill 解析、frontmatter、ignore 规则全归 core）；不复刻 pi 的 `resources_discover` hook——runner 本就 per-agent，reload 重建 runner 时重新激活即重新声明，无需动态 discovery 事件。
- **Scope**：own-agent overlay。贡献只进本 agent 的 `<skill:>`/`<prompt:>` 候选与展开、spawn 装载和 system prompt skills 列表，不扩展 global roots，不影响其他 agent。
- **冲突语义**：first-registration-wins + diagnostic。core 侧 profile/cwd 资源先注册故必胜，extension 贡献按激活顺序解析；同名贡献被丢弃并发 `extension.resource_conflict` diagnostic（含 resourceType、name、path、extension 归因）。不用 command 的 rename-with-provenance——skill/template 名在声明与调用之间不许变形。
- **Provenance**：贡献声明（extensionId + 路径）进 `ExtensionRunnerSnapshot.resourceContributions`；resolved provenance（每个已装载资源的名字与来源 root/extension）进 agent snapshot 的 `resources` 事实。加载失败与冲突都以 diagnostic 呈现，进 record 的 resourceDiagnostics。
- **Stale 语义**：与 inline command 同族——stale runner 的贡献退出运行时装载管线（候选、展开、按名取用），spawn 期装载的 harness resources 不回溯变更。
- 锚点 consumer：`tests/extensions/resource-extension.ts` 只声明路径，消费方是既有 core 管线（`<skill:>`/`<prompt:>`、候选列表、system prompt），集成回归覆盖贡献装载、冲突丢弃、provenance 与 stale 退出。

### Extension provider contribution（ME 切片 9）

`registerProvider(providerName, config)` 在激活期声明一个 model provider（模型集、baseUrl、request auth 配置、可选 OAuth 桥接与 custom `streamSimple`）：

- **Scope：global + provenance，生命周期绑 runner。** 模型表与 models.json 同级，是进程全局共享资源，own-agent overlay 需要 per-agent Models 包装、侵入 pi-ai runtime 消费方式——与 scope-by-default 的差异是显式裁决。注册按 (extension, agent) 记账：reload 先撤销 stale runner 的注册再由新 runner 重声明；agent dispose 撤销其注册，同一 extension 被多个 agent 激活时按 registrant 引用计数，最后一个 agent 退出才移除 provider 并重建 runtime。
- **冲突语义：只许新 provider 名，first-registration-wins。** 内置 provider、models.json provider、runtime 动态注册与他人 extension 的名字一律不可覆盖，冲突 drop + `extension.provider_conflict` diagnostic（含 conflictWith 与 owner 归因）；`refresh()` 重放时重查同一规则，models.json 后来认领名字时 extension 注册被丢弃并发同一 diagnostic。pi 的 provider override（`registerProvider("anthropic", { baseUrl })` 代理场景）不收编——用户自己的 override 通道是 models.json，extension 静默改写内置流量去向正是 provenance 纪律要防的事；受控 override 入口等举证。推论：extension 注册必须带完整 models（新名字下没有既有模型可 override），override-only 形态在校验层拒绝（`extension.provider_invalid`）。
- **Auth 所有权不移交。** `apiKey` 是 config value 引用（literal / `$ENV` / `!command`），由 core 在请求期经 ConfigValueResolver 解析；OAuth credential 持久化在 core AuthStorage，refresh 走 pi-ai `Models.getAuth()` 的 locked refresh。extension 声明 `oauth`（login/refreshToken/getApiKey/modifyModels 回调）即可接入既有桥接；login 的发起面（/login command 或 client host）尚不存在，举证缺口见 BACKLOG。
- **Trust gate。** provider config 的任何 `!command` config value（apiKey、provider/model headers）在 project trust 未通过时整条注册被拒（`extension.provider_trust_denied`），与 scoped `exec` 门控同族——core 不替 untrusted 项目里的 extension 执行命令；literal 与 `$ENV` 不受限。
- **Provenance 进 inspect facts。** 贡献声明（extensionId、providerName、modelIds、oauth 标志——不含 config value，literal apiKey 是 secret）进 `ExtensionRunnerSnapshot.providerContributions`；registry 侧 `getExtensionProviderRegistrations()` 暴露 name/owner/registrant agents。
- **注册形态：激活期声明唯一。** reload 重激活即重注册；不提供手动 `unregisterProvider`（生命周期自动收尾使其无场景），不做运行期动态注册（pi 的 pending-queue 复杂度随之消失），有 consumer 举证时再评估。
- **Spawn 边界**：spawn/resume 的默认模型解析先于 extension 激活，贡献模型不能作为 spawn 默认模型；注册在 harness 创建前完成，首轮起即可经 `setModel`//`model` 选用。
- 锚点 consumer：`tests/extensions/provider-extension.ts`（model-gateway 形态：注册 provider + `before_provider_request` 盖章），集成回归覆盖注册可用、四类冲突、trust gate、per-agent 生命周期、reload 撤销/重注册与 request patch 合成。
...
当前结论：**作者 API 面已冻结（API v1，ME 切片 10）：第三方 extension 可依赖 `extension/api.ts` 契约开发；产品级交付面仍缺 client adapter 的 extension host 与 product presentation。**
...
已随 ME 切片 10 收口：注册面（`registerTool`、activation-time `patchTool`、`registerCommand`、`contributeResources`、`registerProvider`）齐备，作者契约冻结为 `extension/api.ts`（API v1），版本兼容与 `extension.version_incompatible` 语义定案（见公开契约节）。

仍缺：

- Hook event matrix 的 backlog 项：`before_provider_payload` 与可取消 session hook 等举证（canonical facts 与六个 interceptor 已落地）。
- Product presentation：`agent.inspect` 已有 facts，但还没有产品级 UI/RPC 呈现。

ME milestone（hook matrix、provider/resource registration、extension-owned storage、API 面冻结）已全部落地（2026-07-13），总方案与逐切片裁决见 [Extension Experiment](./extension-experiment.md)；product presentation 移入 backlog 随 client adapter 举证。等 client adapter 的 extension host 就绪后，再把 team/flow/goal 类 extension 作为 product surface；coding tools 已裁决为 core built-in（见 DESIGN.md），不再依赖 extension 形态交付。
