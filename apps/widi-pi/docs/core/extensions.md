...
版本策略：单调递增整数，只在冻结面破坏性变更时 bump；runtime 支持区间为 `[MIN_SUPPORTED_EXTENSION_API_VERSION, EXTENSION_API_VERSION]`（当前 `[1, 1]`）。声明形态统一三种 extension source（factory/file/package）：模块默认导出或 `registerExtensionFactory` 传 `{ apiVersion, activate }`。裸 factory 视为面向当前版本——仓库内 extension 与 runtime 同步演进，强制声明只有噪音；第三方发布应当显式声明。
