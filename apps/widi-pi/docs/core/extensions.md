...
这些能力足够验证 ToolRegistry、首批 hook matrix、diagnostics、reload、input command、session custom entry、resource contribution 和 provider contribution 的主路径。仍不足以作为稳定第三方 extension surface：可取消 session hooks、`before_provider_payload`、product presentation 和完整 RPC adapter 尚未收口（API 面冻结归切片 10）。
...
- Extension API：注册面（`registerTool`、activation-time `patchTool`、`registerCommand`、`contributeResources`、`registerProvider`）已齐；差距在契约冻结与版本兼容（切片 10）。
- Hook event matrix：canonical command/human/diagnostic/agent/session/input facts 与 raw harness observer 已落地，六个 interceptor（五个 Pi harness hook + WIDI 原生 `input`）已标明 intercept/mutate 档位与失败语义；`before_provider_payload` 与可取消 session hook 留在 backlog 等举证。
