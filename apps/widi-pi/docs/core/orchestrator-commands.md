...
当前 `core/orchestrator/*` 仍保存 command、client、human-request 和 diagnostic 的类型定义，运行时调度、pending human request、client fanout 也仍由 `AgentOrchestrator` 直接拥有。runtime-service 成型后，Command 应从 `AgentOrchestrator` 内部逻辑中抽出，成为直接操作 runtime-service/facade 的 runtime module，而不是 `AgentOrchestrator` 的子模块。

## Commands

TUI、stdout、RPC、tool 和 extension 不应直接操作 `AgentHarness`。它们向 Command 提交命令式请求，例如 `agent.prompt`、`agent.steer`、`agent.nextTurn`、`agent.getModel`、`agent.setActiveTools` 或 `human.request`。Command 再通过 runtime-service/facade 操作 agent、human request、session、extension 和 diagnostics。
...
当前 `OrchestratorCommand` 已经承担 runtime operation bus；`agent.input`、built-in `inputInvoke` 和 extension `registerCommand()` MVP 让 command definition / contribution / resolve / execute 的形状开始出现。下一步应把这些散落逻辑收敛成 `Command` runtime module。
...
- `Command.execute(command)`：执行 typed command，并通过 runtime-service/facade 产生 runtime effect。
...
内置 command 应通过 executable definition 映射到 typed command behavior。例如 `/compact foo` 可解析为 `agent.compact` command，再由 Command 通过 runtime-service/facade 执行。Extension command 没有固定 built-in kind 时，可以保留 handler，但 handler 只能拿 command context facade 和 runtime services，不能持有 raw orchestrator、raw harness、agents map 或 registry mutation surface。
...
3. execute：Command 调用 runtime-service/facade 执行 resolved command，发布 command lifecycle events 和 diagnostics。
...
- Command 直接操作 runtime-service/facade；它不只是输出 runtime object，也不把执行逻辑转交给 harness。
...
这些调用不应通过 raw `AgentOrchestrator` 暴露。Core 应提供受控 runtime services facade：

- `commands.dispatch(...)`：提交 typed orchestrator command。
- `agent.spawn/prompt/wait/status/inspect/...`：agent lifecycle 和 A2A facade。
- `human.request(...)`：结构化人机请求。
- `session.appendEntry/findEntries(...)`：当前 extension namespace 的 session-local state。

Facade 由 runtime-service 创建并绑定当前 agent/profile/extension/source。不同上下文可以拿到不同能力子集：tool execution、command handler、observer、interceptor 和 adapter 不必拥有相同权限。所有 facade 操作仍回到 runtime-service 边界，统一产生 diagnostics、events、policy checks 和 lifecycle effects。
...
Orchestrator command 后续任务集中维护在 [WIDI 下一阶段 TODO](../TODO.md)。本文件只保留 command/client/human-request 的语义边界。
