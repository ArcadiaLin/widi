...
本文记录当前 orchestrator command/client/human-request 的边界事实。Command 的长期形态仍是实验性设计，不应按本文早期版本理解为已经稳定的最终架构。实验设计单独记录在 [Command Experiment](./command-experiment.md)。

`core/command` 目前保存 command 类型、built-in input command、input parser 和 command execution。`AgentOrchestrator` 仍是当前 multi-agent runtime object，负责 agent lifecycle、pending human request、client fanout 和 harness events。

Command 不是独立通信层，也不是 mandatory façade。Runtime consumer 可以使用 `Command`，也可以直接使用 orchestrator 原子能力。

## Commands

TUI、stdout、RPC、tool 和 extension 不应直接操作 `AgentHarness`。当前建议路径是向 orchestrator/Command 提交命令式请求，例如 `agent.prompt`、`agent.steer`、`agent.nextTurn`、`agent.getModel`、`agent.setActiveTools` 或 `human.request`。Command 再直接通过 orchestrator runtime 操作 agent、human request、session、extension 和 diagnostics。
...
当前 `CommandRequest` 承担 runtime operation request；`agent.input`、built-in `inputInvoke` 和 extension `registerCommand()` MVP 已经收敛到 `Command` runtime module。
...
内置 command 目前由 built-in input command 数组映射到 typed command behavior。Extension command 没有固定 built-in kind 时，当前 MVP 保留 handler，并沿用 extension context；后续再决定是否需要更窄的 command context。
...
当前阶段不引入强制 facade。`AgentOrchestrator` 就是 multi-agent runtime object，Command 和未来 product tools 可以直接依赖它，就像 Pi coding-agent 的 interactive layer 直接组合 `AgentSession` 能力一样。

如果后续出现真实的稳定性压力，再从实际调用点抽取小 facade；不要预先为了隔离而复制 runtime service surface。

## Export Boundary

Command 如果稳定下来，应作为可选导出，而不是主 runtime 的唯一入口。例如 consumer 可以选择：

```ts
import { Command } from "@widi-pi/command";
```

使用者也可以完全不导入 Command，直接控制 orchestrator 的原子化行为。这条边界避免实验性 command runtime 过早变成强制 public API。
