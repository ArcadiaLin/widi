...
这说明 orchestrator 已经接入第一版 profile registry contract，但 resource、extension、tool 和 diagnostics 汇总仍未完成。

## 主要缺漏

### Profile 加载

`AgentProfileRegistry` 已经替代 `AgentProfileLoader` 作为 profile 入口。Registry 持有 storage backend，按 `ProfileId` 建 lazy metadata index，并处理 source priority、冲突和 profile diagnostics。
...
仍未完成的是 runtime composition 接入真实 file roots、settings paths 和 builtin default source，以及 diagnostics 的统一汇总/展示策略。
...
因此后续 orchestrator 接入时应把 `profile.tools` 传给 registry，把 `session.buildContext().activeToolNames` 传给 registry，并把 registry diagnostics 合并进 orchestrator diagnostics。`SpawnAgentHarnessOptions.tools` 应逐步收敛为测试/adapter escape hatch，或转成 adapter contribution 后再进入 registry。
...
- [x] 实现 `AgentProfileLoader`，从 agent dir 与 project `.widi/profiles` 加载 profile 并返回 diagnostics。
...
- [ ] 扩展 `OrchestratorDiagnostic`，覆盖 profile、resource、extension、tool 和 model 相关问题。
