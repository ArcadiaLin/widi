...
Tool execution 的 UI 展示也不属于 profile schema。Orchestrator 会原样发布 Pi `AgentHarnessEvent`，UI/RPC/extension runner 可从 `agent_harness_event` 中的 tool name、arguments 和 result details 派生展示数据。
