...
目标公式、pi 能力对照表、形状差异示例、裁决原则与切片细节见 [Extension Experiment](core/extension-experiment.md)。锚点 consumer：审计/策略 extension（consumer 举证均指向它，除注明者外）。原 M4 的设计条目全部收编于此；ME 实施所需的 M2 地基项（切片 0/1/2）已直接迁入：
...
- [x] 切片 5：Hook matrix 第一批（observe 档）。已落地：canonical `command_*`、agent-scoped `human_request_*`、diagnostic、`agent_spawned/resumed`、`agent_session_info_changed/forked` 经 `_emit()` 唯一桥接到 own-agent runner，`observe(name)` 按事件名类型收窄；global 事实不向 per-agent runner 复制，observer 期间产生的 diagnostic 不回灌，避免失败递归。compact/tree/model/thinking 沿用唯一 raw harness event 口径；session switch/shutdown 因当前无真实语义明确不造伪事件。Hook matrix 已逐项标 observe/intercept/mutate，审计 consumer 覆盖 canonical 账本、双 agent 隔离和失败语义；裁决全文见 extension-experiment.md 切片 5。
- [x] 切片 6：`input` interceptor（已裁决：拦截在 command 解析之前，改写后重走完整解析与 gateway）。已落地：`intercept("input")` 管线（`context` 式改写 + `tool_call` 式短路），fail-closed；触达 `inputAgent` 全路径含 commands 禁用短路；不做 pi `handled`；`input_transformed`/`input_blocked` canonical 事实 + `InputResult.blocked` 形态；审计 consumer 增补 `denyInput` 账本。四项落地裁决全文见 extension-experiment.md 切片 6。
- [x] 切片 7：Extension-owned storage 裁决 + custom entry policy（fork/compaction/export/`custom_message`）；extension inline `expand` 契约顺带接入。已落地：custom entry 定为 core 唯一 storage 通道并升格成文契约（fork 分支事实、compaction 零影响、export 归 client）；`custom_message` 不收编（差异接受 + backlog 举证缺口）；input 改写落 `core:input_transform` dual-record（block 不落）；`registerCommand({ placement: "inline", expand })` 接入统一扫描/gateway/dual-record 管线，`expand(argument)` 无副作用靠形状强制。四项裁决全文见 extension-experiment.md 切片 7 记录。
- [x] 切片 8：Resource contribution（skills/prompt templates 贡献，registration-with-provenance，ResourceLoader 所有权边界）。已落地：`contributeResources()` 激活期声明路径（ResourceLoader 仍是唯一 FS 读取者，不复刻 pi `resources_discover` hook）；own-agent overlay，贡献只进本 agent 的 `<skill:>`/`<prompt:>` 候选、展开与 spawn 装载；冲突 first-registration-wins + `extension.resource_conflict` diagnostic（core 侧 profile/cwd 必胜，名字不变形）；贡献声明与 resolved provenance 均进 inspect facts；stale runner 贡献退出运行时管线；不建 ResourceRegistry 类。锚点 consumer `tests/extensions/resource-extension.ts`（真实 product consumer 推迟至 pi-tui adapter 阶段）。裁决与落地全文见 extension-experiment.md 切片 8 记录。
- [x] 切片 9：Provider contribution（ModelRegistry register/unregister provider；auth 所有权不移交；provider hook 视 pi harness 暴露评估）。已落地：`registerProvider()` 激活期声明，global + provenance、生命周期绑 runner（reload 撤销重注册、dispose 引用计数收尾）；只许新 provider 名（override 通道归 models.json，pi 代理形态差异接受）；OAuth 配置收编、login 面入 backlog；`intercept("before_provider_request")` patch 管线（失败跳过）+ `after_provider_response` raw 可达落定，`before_provider_payload` 入 backlog；`!command` config value 受 project trust 门控。锚点 consumer `tests/extensions/provider-extension.ts`。六项裁决全文见 extension-experiment.md 切片 9 记录。
- [ ] 切片 10：API 面冻结：公开契约清单、版本兼容策略、`extension.version_incompatible`；第三方视角验收 extension。
