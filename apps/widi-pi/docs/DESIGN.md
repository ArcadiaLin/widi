...
- client fanout 与 human-request 已形成 orchestrator 的受控输出语义；`human-request.ts` 已成为 orchestrator 挂载的 runtime collaborator，拥有 pending/timeout/cancel 生命周期；command 层实验已裁决——trigger-based command input 是 orchestrator 自身的 input 能力，独立 command runtime 不再存在（见 [Command Experiment](core/command-experiment.md)）。
- 七个 core coding tools 已通过 tool registry/orchestrator 主路径落地；agent collaboration tools 进入 M3 设计落地阶段。
...
裁决：coding tools（read/write/edit/bash/grep/find/ls）是 **core built-in tools**，以 `source: core` 注册进 ToolRegistry，实现尽量复刻 pi-coding-agent 的工具语义与 result 形态（arguments/content/typed details）。七个工具现已按固定顺序注册，active tools 的 `promptSnippet`/`promptGuidelines` 进入 system prompt composition；read 同时支持 provider-compatible 图片结果与全局禁图 policy。理由：skill、prompt、profile 都是文件形态资源，read 是 runtime 自身依赖（`/skill` 的展开产物指引模型用 read 加载 skill 正文）；有读则写/编辑顺带。曾经的 "extension-contributed" 结论撤销——extension 的组合自由不受影响，它仍可通过 `patchTool` 改写任一 built-in tool 的 backend（sandbox、远程、审计包装），这正是 ToolRegistry patch 管道存在的意义。未来按产品事实再评估 built-in 集合的裁减。

### Auto-compaction（已落地）

Compaction 长期只有手动 `/compact` 一个入口：settings `compaction.enabled/reserveTokens/keepRecentTokens` 一直存在但没有 runtime 消费者，长会话必然撞 context window（2026-07-14 复核发现）。

裁决：阈值触发归 orchestrator，不归 harness、不归 client。orchestrator 在 harness `settled` 事实上检查（此时 harness idle 且 pending session writes 已落盘；`nextTurnCount > 0` 跳过，避免与下一轮 run 的 busy 竞态），用当前 branch 最后一条 assistant usage 对照 `model.contextWindow - reserveTokens`（上游 `shouldCompact`/`calculateContextTokens` 同一口径），超阈值调用 `compactAgent`。失败降级为 `compaction.auto_failed` warning diagnostic，不抛错——无法压缩的超限会话保持原有行为（跑到 provider 拒绝为止）。

已知限制：`keepRecentTokens` 设置只在 WIDI 侧存在，上游 `AgentHarness.compact()` 内部硬编码 `DEFAULT_COMPACTION_SETTINGS` 决定切点；两侧默认值一致（20000），但用户改配置不会影响实际保留量。需要上游为 `compact()` 增加 settings 参数才能打通（与 message_end PR 同属 upstream 对齐轨道）。
...
Core 直属模块按 runtime 责任归位，而不是按“类型文件/实现文件”二分。跨 core 共享协议事实进入 `core/types.ts`；其他类型跟随所有者模块。完整布局纪律见 [Runtime Modules](core/runtime-modules.md)。
...
`human-request` 是 core runtime module，由 orchestrator 挂载和授权。它目标为 human-facing client，可能等待 human response。

`HumanRequestBroker` 拥有 request id、pending map、timeout、abort、cancel 和 `human_request_*` event shape。Orchestrator 只提供 narrow host：选择 client、emit event、publish diagnostic、记录 agent dispose 期间的取消失败。这样 human-request 成为可测试、可替换的 runtime collaborator，而不是 `AgentOrchestrator` 内的一段内联状态机。
...
- [Runtime Modules](core/runtime-modules.md)
