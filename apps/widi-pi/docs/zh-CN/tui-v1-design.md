...
selector；selector command 与 agent selector 复用 editor 上方的 inline
...
第一版仍允许尚无专用界面的 built-in command 通过普通 command 输入执行，并以通用 command result 或 human request 呈现。带 `arguments.complete()` 的 line command 可以在提交 bare command 时复用通用 inline completion menu；这不改变 command 的 core 执行路径。
...
后台 `extension_output` 必须进入对应 agent timeline 并增加 unread count，但它本身不提升 `attention`：运行态仍由 status 表达，随后到达的 `command_completed` 再按既有规则产生 completed attention。切换到该 agent 后清除 unread；不得把 output 渲染到当前 active agent，也不得抢占 focus。
...
- `Command.arguments.complete()` 在用户进入参数位置后提供 autocomplete；
  提交无参数的 bare line command 时，TUI 也可调用同一 contract，并在 editor
  上方打开 completion menu。
...
- `select` free input、`input`、`argumentsCompletion`、`custom` 一律显示 `Answered`，不回显 response value。
...
`reportDiagnostic` 产生的每次上报都有 fresh core id，因此即使 code 相同也表示多个独立事实，TUI 不按 code 合并。它与 load/runtime diagnostic 走同一 `DiagnosticItem` 与 attention 规则；`commandId` 只用于关联展示或调试，不改变 severity/disposition。
...
5. 锁定 extension line-command return passthrough 与 append-only `extension_output`；output 发送给 listeners/clients，但不进入 observer 或 session。（已完成）
...
3. 锁定 mutation-first、missing clear no-op、commandId correlation 与 observer no-feedback。
...
3. 锁定 kind/title/content validation、UTF-8 payload bounds、commandId correlation 与 observer no-feedback。
...
3. Core 注入 fresh id、extension/agent/profile attribution、规范化 code 与可选 commandId。
...
2. Core 注入 `presentationId`、agent/extension attribution 与可选 `commandId`。
...
4. 处理 command result、`extension_output`、blocked/transformed input 和运行状态规则。
...
- extension command 返回值进入 `command_completed.result`，`undefined` 不创建 item。
...
- bare selector command 打开 inline completion menu，选择后走正常 submit，
...
- extension line-command return 同时进入 `InputResult.value` 与 `command_completed.result`。
- 顺序 await 的多次 `emitOutput()` 按调用顺序位于 `command_accepted` 与 `command_completed` 之间。
...
- `notify()` 产生带 presentationId/agentId/extensionId/commandId 的 `extension_notification`，顺序调用位于 command accepted/completed 之间；event 不进 observer/session/model context。
...
- 成功 reload/dispose 清空 status，skipped/failed reload 保留；command completion 不自动清空。
...
- `reportDiagnostic` 注入 fresh id、规范化 code、agent/profile/extension attribution 与 commandId；重复 code 的多次调用保持为独立事实。
...
启动 WIDI TUI
  ...
  -> 执行 extension command，看到增量 output 与最终 command result
