...
状态：v1 基础实现已落地；保留本文作为实现规格与人工验收清单。v2 交互裁决已并入对应章节（以「v2 已裁决」标注）并已实施；仅依赖 pi-tui 上游能力的条目仍暂缓（alternate screen、SelectList disabled 样式，见 §18；`app.steer` 的「取出排队 followUp」语义见 §11.2 实现注）。未标注的条目反映当前实现。

当前实现入口为 `npm run tui`，代码位于 `src/cli.ts` 与
`src/tui/`。`src/cli.ts` 是唯一命令入口（与 pi-coding-agent 相同形态：
cli 直接导向 TUI），原最小 line CLI 已随 v2 落地移除。TUI 现已包含
application coordinator、纯状态 projector、current
branch hydrator、command autocomplete、editor 上方停靠的 human-request
completion menu、连续 transcript、extension 六通道投影、顶部 processing
栏、editor 上方 queued input 区、footer、响应式 agent strip 与 agent
selector；需要参数的 command 与 agent selector 复用 editor 上方的 inline
completion menu，悬浮 overlay 只保留给致命错误。
...
```text
header / processing 栏（v2）
startup notices
chat transcript
working status
queued input 区（v2，editor 上方的 followUp 队列）
inline completion menu（按需；v2 起同时承接全部 human request）
editor + autocomplete（autocomplete 在 editor 下方即打即现）
footer
agent strip
```
...
v2 已裁决：TUI 不使用悬浮 selector。一切「从候选中选一个」的交互（needs-argument、agent selector、human request）都由 editor 上方的 inline completion menu 承接；editor 打字触发的 autocomplete 固定在 editor 下方临时展开。两者同开时后开者持有焦点，Esc 逐层关闭并归还。悬浮 overlay 只保留给导致程序无法继续运行的致命错误（见 §13.1）。Alternate screen（进入空白全屏、退出交还 shell）依赖 pi-tui 上游支持，上游提供前不实施（见 §18）。
...
`src/cli.ts` 是唯一命令入口：解析 `--cwd` / `--agent-dir` / `--profile` 后直接导向 TUI（cwd 默认继承终端 cwd）。v2 落地时移除了原最小 line consumer 与 `tui-entry.ts`，package binary `widi-harness` 指向 `dist/cli.js`。
...
位于正常 component flow。v2 已裁决：human request 也由 completion menu 承接（见 §12），`human-request` mode 指 menu 承接请求时的焦点态；capturing overlay 只保留给致命错误（见 §13.1）。
...
v2 已裁决：human-request trace 是 resolved completion menu 的可折叠留痕，与 tool output 共享 `app.tools.expand`（ctrl+o）全局展开/合并。默认合并为一行；展开后 `confirm` 显示 Yes/No 两个原选项，`selected-option` 显示全部原始 options 并高亮所答项（因此 item 需保留请求时的 options 供展开渲染）；`answered`（input/custom/free input）永远没有展开内容。隐私规则不变：select 值必须属于原始 options 才显示。
...
TUI
  ...
  ChatContainer
  StatusContainer
  QueuedInputContainer   （v2 新增）
  CompletionMenu
  EditorContainer
  Footer
  AgentStrip
  Overlay: FatalErrorView （v2 起仅致命错误）
```

v2 已裁决新增两个 component：

- `ProcessingBar`：仅 active agent running 时显示，内容为正在处理的用户消息（单行截断）+ 当前活动（thinking / tool name / steer 注入）+ elapsed。事实全部来自 projection（user item、`tool_execution_start`、thinking status），不需要 core 新事件。窄终端降级为只显示 spinner + elapsed。
- `QueuedInputContainer`：running 时提交的 followUp 队列显示区（见 §11.2），多条按序排列并标注去向；与 footer 的 queue 计数同源。
...
`Editor` 始终是默认 focus target。Completion menu 或 overlay 关闭后必须显式恢复 editor focus。两个菜单的位置规则（v2 已裁决）：completion menu 固定在 editor 上方槽位持续展开；editor autocomplete 在 editor 下方即打即现；同开时后开者持有焦点，Esc 逐层归还。
...
- `extension_status_changed` 不创建 timeline item。Active agent 的状态在 `StatusContainer` 中按 key replace-in-place；有 `total` 时显示 determinate progress，无 `total` 时显示 spinner/计数。非 active agent 只保留 projection，agent strip 最多显示 neutral 摘要或 `+N`，不得把 status 当成 warning/error attention。v2 已裁决：status 槽位每项追加 `updatedAt` 年龄显示，相对时间格式（`· 12s`，60 秒以上 `· 3m`）——command 结束不自动 clear 是既有契约，年龄是用户判断 status 是否陈旧的唯一手段；`updatedAt` 已在 projection 中，纯展示层改动。
...
- description：status、model、elapsed/unread 与 attention reason。v2 已裁决：attention reason 使用人话短句（如 `needs input`、`spawn failed`），不显示裸枚举串。
...
Pi-tui 必须在 extension 或 core 行为可能发起 human request 前完成初始化，否则 handler 没有可用的 menu host。
...
后台完成不抢占 focus。后台 human request 升该 agent 的 human-request attention 并出现在 menu tab 行（menu 未开时显示待答提示），标题必须显示 agent identity；回答后回到原 active agent 和 editor draft（v2 已裁决，见 §12）。
...
Pi `AgentHarness.prompt()` 在 busy 时会失败，而 steer、follow-up 和 next-turn 是不同语义。v2 已裁决（替代 v1 的「文本保留在 editor + notice 提示」规则）：
...
v2 已裁决：human request 的呈现从「capturing overlay 唯一形态」改为「completion menu 承接」。本节 core 契约（broker、envelope、响应类型、provisional、timeout/cancel）不变，只修订呈现层。

### 12.1 Queue

TUI 注册一个 human request handler，内部维护 pending queue。v2 已裁决：呈现上不再严格 FIFO——每条 pending 请求是 completion menu 的一个实例，方向键聚焦到 menu 后，←/→ 在所有等待中的请求间切换（menu 顶部 tab 行显示 agent 与 kind），↑↓ 在当前请求的选项间移动，Enter 只回答当前聚焦的请求；各请求 promise 独立，回答顺序不限。
...
所有 kind 复用 `CompletionMenu` 组件、配色与 `tui.select.*` 按键，位于 editor 上方槽位；标题行固定携带 source、requesting agent label 与 kind。menu 打开时 editor 不提交。

- `confirm`：呈现为 Yes/No 两个选项，Esc 返回 `confirmed: false`。
- `select`：选项列表（`allowFreeInput` 时含 free-input 入口），Esc 返回 `value: undefined`。
- `input`：menu 位置内嵌输入行，空值按 contract 返回 `undefined`。
- `custom`：第一版只支持可安全 fallback 为文本或 option 的 payload；无法解释时显示 diagnostic-friendly notice，并返回 `{ kind: "custom", value: undefined }`。

多步流程原地变换（v2 已裁决，/login 是典型场景）：同一流程的连续请求复用同一槽位，内容随流程推进 replace-in-place（select → 展示 URL/设备码 + 输入行 → waiting/provisional），而不是每步开关一次菜单。waiting 态显示 spinner 与说明文字且输入行保持可用；provisional 请求被 core 撤下时 menu 静默关闭、焦点归还 editor。多步内容只存在于 menu 生命周期，不进 transcript。

后台请求不全局弹出、不改变 active agent：升该 agent 的 human-request attention；menu 已开则 tab 行出现新项，否则提示待答请求数。`app.request.open` 快捷键（见 §14）直接跳答最近的后台请求。answer 完成后清除其 human-request attention，如果还有 unread/diagnostic 则按更低优先级状态继续显示。

Resolved request 在所属 agent timeline 追加一个 ephemeral `HumanRequestTraceItem`（v2 起为可折叠 item，见 §6.3），只保留隐私安全摘要：
...
- Signal abort：立即从 queue 移除，关闭对应 menu 实例，并聚焦下一项。
- Core timeout/cancel event：清除对应 request UI，不再提交迟到 response。
- Agent dispose：core 取消该 agent pending request，TUI 只消费结果 event。
- Application shutdown：先关闭 local queue，再 unregister client，避免 cleanup 过程中打开新 menu。
...
- v2 已裁决：悬浮 overlay 只承接「导致程序无法继续运行」的致命错误，且仅两类——(1) 启动失败：runtime 或初始 agent 创建失败、不存在任何可用 agent；(2) 未捕获的致命 application error：application 边界捕获、状态可能损坏、不应继续运行。两者以模态 overlay 呈现错误摘要与 code，操作为 Quit / View diagnostics（后者关闭 overlay 只读查看诊断视图）。明确排除：后台 agent spawn 失败（strip 呈现）、hydration 失败（notice）、`disposeAll` 失败（正在退出）、单个 command 失败（command result 行）。human request 不再使用 overlay（见 §12）。
...
- `app.tools.expand`：`ctrl+o`，全局切换 tool output 与 human-request trace 的展开/合并（v2 起 trace 共享此开关，见 §6.3）。
- `app.steer`：`ctrl+s`，running 时立即 steer（见 §11.2 实现注）。
- `app.request.open`：`ctrl+r`，跳答最近的 pending human request（见 §12）。
- `app.request.previous` / `app.request.next`：`left` / `right`，request menu 聚焦且当前请求不是输入行时在等待中的请求间切换。
...
- running agent 普通输入进入 followUp 队列并显示在 queued input 区（v2）。
- `app.steer` 把最新排队 followUp 取出改作 steer（v2）。
- running agent 的已知 line command 仍进入 `CommandEngine`。
- idle agent 的未知 `/typo` 先显示 notice，二次 enter 才提交（v2）。
...
- 多请求并存：tab 行出现、←/→ 切换、Enter 只回答当前聚焦请求（v2）。
- confirm/select/input/custom。
- 多步流程原地变换与 provisional 撤下（v2）。
- signal abort、timeout、agent dispose 与 application shutdown。
- request menu 关闭后 focus 恢复。
- resolved trace 的 confirm/select 安全摘要、input/custom 隐私隐藏与 ctrl+o 展开（v2）。
...
- Alternate screen 与 autocomplete 不可用项置灰：v2 已裁决为上游依赖——pi-tui 提供 TUI smcup/rmcup 生命周期与 SelectList disabled 样式前不实施，WIDI 侧不自行绕行；上游具备能力后按 §2.4 与 §11.3 的目标形态启用。
