...
- `extension_output` 没有 core output id；projector 按每个 agent 的事件接收顺序分配单调递增的 local view key。不得只用 `createdAt`，同一毫秒可以收到多条 output。
...
| `extension_output` | 按 `agentId` 追加一个 ephemeral `ExtensionOutputItem`，分配 local view key | active chat 显示带 extension id 的 plain text；background agent 只增加 unread |
...
5. 锁定 extension line-command return passthrough 与 append-only `extension_output`；output 发送给 listeners/clients，但不进入 observer 或 session。
...
- `extension_output` 逐 event 追加、同毫秒事件 local key 不冲突、不会回灌 observer。
