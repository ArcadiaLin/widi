...
交互层先完成 inline expansion，再调用 `promptAgent`；模型看到展开并可能经 extension transform 后的文本。人类原始输入与 expansion/transform 位置仍可在 resume/fork 后恢复。Programmatic caller 不能绕过 `promptAgent` 的 extension input interception。
