...
7. 成功发布 `command_completed`；执行前拒绝发布 `command_rejected`；执行中失败发布 `command_failed`。
...
- Scoped actions：own-agent model、thinking、tools、input、human request、session info、exec、abort、compact 等受控能力。
...
Canonical orchestrator facts（command、human request、diagnostic、agent/session、input）经同一 `_emit()` 路径发送。Observer failure 不改变原操作结果；它产生 `extension.handler_failed` diagnostic。Diagnostic observer 处理中产生的新 diagnostic 不回灌 extension observer，避免递归。
