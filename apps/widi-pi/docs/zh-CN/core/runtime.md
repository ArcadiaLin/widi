...
7. 成功发布 `command_completed`，其 `result` 与 `InputResult.value` 使用同一返回值；执行前拒绝发布 `command_rejected`；执行中失败发布 `command_failed`。Extension handler 可以在完成前顺序发布 append-only `extension_output` 或 info-only `extension_notification`。
