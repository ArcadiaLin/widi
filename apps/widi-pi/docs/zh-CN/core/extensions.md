...
- `ExtensionStatus`、`ExtensionStatusProgress` 与 `ExtensionStatusSnapshot`。
...
`extension_output` 与 `extension_status_changed` 有意不属于 observed event：extension 主动发布的 presentation facts 不会再次进入任何 extension observer，避免反馈与递归。
