...
3. Orchestration layer
   ...
   `AgentOrchestrator` 组装 agent runtime，管理 lifecycle、command input、client fanout、human request、diagnostic publish 和跨 agent 协作。

4. Application layer

   CLI、TUI、RPC、product preset 与 client-side extension host。该层负责具体交互和呈现，不反向拥有 core state。
...
Programmatic consumer 直接调用 orchestrator 原子方法。Command 只表示由人类输入触发的协议；`inputAgent` 负责解析、门控、参数补全、inline expansion 和统一事件。Core 不维护镜像原子方法的 typed command runtime。
...
Extension 作者契约冻结为 API v1。Extension 可以注册或 patch tool、注册 line/inline command、贡献 resources/provider、观察或拦截 runtime、调用 own-agent scoped actions，并使用 namespaced session custom entry 保存小型 session-local 状态。UI 渲染和快捷键属于 client adapter。
...
Orchestrator 在 harness `settled` 后按 context threshold 触发自动 compaction；失败发布 warning diagnostic，不改变原有运行语义。实际保留 token 数仍受 Pi `AgentHarness.compact()` 的上游设置边界约束。
