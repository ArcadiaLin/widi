...
这是临时路径规划入口，不是长期协议。后续如果 multi-agent run store 稳定下来，路径很可能转向 run 级结构，例如：

```text
<runRoot>/agents/<agentId>/session.jsonl
```

或者：

```text
<runRoot>/sessions/<agentId>.jsonl
```
...
- run 级 session index。
- mailbox、agent lifecycle、checkpoint 等 multi-agent run store。
- 删除 session 时与未来 run registry 的同步。
- 与 Pi upstream JSONL storage 的自动兼容测试。

这些问题都应等 orchestrator、profile registry、runtime service 和 run store 的边界更稳定后再收敛。
...
3. 抽象为 run/session persistence backend

   如果 multi-agent 架构最终需要 run store、mailbox、checkpoint 或数据库后端，可以保留 Pi 的 `Session`/`SessionTreeEntry` 语义，把 JSONL 文件实现藏在 persistence backend 后面。这样 `AgentHarness` 仍然只看到 `Session`，orchestrator 看到的是更高层的 run/session metadata API。
