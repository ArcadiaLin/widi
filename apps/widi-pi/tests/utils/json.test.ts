...
describe("JSON utilities", () => {
	...
	it("rejects values that JSON cannot represent", () => {
		...
		cyclic.self = cyclic;
		expect(() =>
			normalizeJsonValue(cyclic as unknown as JsonValue, "Test payload", 1_024),
		).toThrow(/Test payload must be JSON serializable/);
	});

	it("applies byte limits to serialized UTF-8, not character count", () => {
		expect(normalizeJsonValue("你", "Test payload", 5)).toBe("你");
		expect(() => normalizeJsonValue("你", "Test payload", 4)).toThrow(
			/Test payload exceeds 4 UTF-8 bytes/,
		);
	});
});
