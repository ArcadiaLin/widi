...
describe("ConfigValueResolver", () => {
	it("resolves literals, env templates, and escapes", async () => {
		...
		await expect(resolver.resolveConfigValue("literal")).resolves.toBe(
			"literal",
		);
		await expect(resolver.resolveConfigValue("$API_KEY")).resolves.toBe(
			"secret",
		);
		await expect(
			resolver.resolveConfigValue(`https://$${"{HOST}"}/$API_KEY`),
		).resolves.toBe("https://example.test/secret");
		await expect(
			resolver.resolveConfigValue("cost=$$5 command=$!echo"),
		).resolves.toBe("cost=$5 command=!echo");
	});
	...
	it("reports env var names and treats empty env values as missing", async () => {
		...
		expect(
			resolver.getConfigValueEnvVarNames("$PRESENT:$EMPTY:$PRESENT"),
		).toEqual(["PRESENT", "EMPTY"]);
		await expect(
			resolver.getMissingConfigValueEnvVarNames("$PRESENT:$EMPTY:$MISSING"),
		).resolves.toEqual(["EMPTY", "MISSING"]);
		await expect(resolver.isConfigValueConfigured("$PRESENT")).resolves.toBe(
			true,
		);
		await expect(resolver.isConfigValueConfigured("$EMPTY")).resolves.toBe(
			false,
		);
	});
	...
	it("executes command config values through ExecutionEnv with cache", async () => {
		...
		await expect(resolver.resolveConfigValue("!token")).resolves.toBe("first");
		await expect(resolver.resolveConfigValueUncached("!token")).resolves.toBe(
			"second",
		);
	...
	it("resolves failed, non-zero, and empty command outputs to undefined", async () => {
		...
		env.setCommandResult(
			"fail",
			err(new PiExecutionError("spawn_error", "failed")),
		);
		env.setCommandResult(
			"nonzero",
			ok({ stdout: "value", stderr: "", exitCode: 1 }),
		);
		env.setCommandResult(
			"empty",
			ok({ stdout: " \n", stderr: "", exitCode: 0 }),
		...
		await expect(
			resolver.resolveConfigValue("!nonzero"),
		).resolves.toBeUndefined();
		await expect(
			resolver.resolveConfigValue("!empty"),
		).resolves.toBeUndefined();
	});
	...
	it("throws useful errors for unresolved config values", async () => {
		...
		await expect(
			resolver.resolveConfigValueOrThrow("!missing-token", "API key"),
		).rejects.toThrow(
			"Failed to resolve API key from shell command: missing-token",
		);
		await expect(
			resolver.resolveConfigValueOrThrow("$ONE", "API key"),
		).rejects.toThrow(
			"Failed to resolve API key from environment variable: ONE",
		);
		await expect(
			resolver.resolveConfigValueOrThrow("$ONE:$TWO", "API key"),
		).rejects.toThrow(
	...
	it("resolves headers and omits unresolved optional header values", async () => {
		...
		await expect(
			...
			'Failed to resolve provider header "X-Missing" from environment variable: MISSING',
		);
	});
